...
export default function UpdateSpecilizationAdmin({id ,refresh , onChange , teacher_id}:{teacher_id:string ,id:string ,refresh:boolean , onChange :React.Dispatch<React.SetStateAction<boolean>>}) {
  ...
  React.useEffect(()=>{
      const getCoursesTitle = async()=>{
        try{
          const response = await Axios.get('admin/get-courses-by-teacher', {params:{teacher_id:teacher_id}})
    ...
    const handleUpdateSp =async (event: React.MouseEvent<HTMLButtonElement>)=>{
      ...
      const response = await Axios.post(`admin/update-specializations/${id}`,{teacher_id : teacher_id,title:title , image:image , is_completed:isCompleted , courses:courseId }
        , {headers: {
...
const handleDeleteSp =async (event: React.MouseEvent<HTMLButtonElement>)=>{
        event.preventDefault();
        setError("")
        setMessage("")
      try{
      const response = await Axios.delete(`admin/delete-specializations/${id}`)
              if( response.data.success === true){
                setMessage(response.data.message)
                router.push(`/dashboard/admin/specilizations`)
              }else{
                setError(response.data.message)
              }
  ...
  return (
    ...
    <React.Fragment>
      <div className="add-course-container d-flex justify-content-between align-items-center">
        <button title='edit-sp' className="edit-button" onClick={handleClickOpen}>
            <AutoFixHighIcon className="edit-icon" />
        </button>
        <button title='delete-sp' className="edit-button" onClick={handleDeleteSp}>
