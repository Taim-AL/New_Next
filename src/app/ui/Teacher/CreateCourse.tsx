...
export default function FormDialog({refresh , onChange}:{refresh:boolean , onChange :React.Dispatch<React.SetStateAction<boolean>>}) {
  ...
  const [pointsToEnroll , setPointsToEnroll] = React.useState<number | null> (0)
  const [pointsEarned , setPointsEarned] = React.useState<number | null> (0)
  ...
  const handleCrearteCourse =async (event: React.MouseEvent<HTMLButtonElement>)=>{
    event.preventDefault();
    setIsPending(true)
    setError("")
    setMessage("")
  ...
  try{
  const response = await Axios.post(`teacher/make-course`,{name:name , description:description , image:image , level:level ,teacher_id:id, point_to_enroll:pointsToEnroll , skills:reqIds,aquirements:aquIds, category_id:selectedCategorys[0].id, points_earned:pointsEarned}
    , {headers: {
